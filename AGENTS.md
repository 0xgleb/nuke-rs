...
- `adrs/` - architectural decision records. ADR 0002 documents why CI runs the
  nix flake checks (supersedes ADR 0001's apt-based workaround).
...
- `secretspec.toml` - secrets the example needs. Loaded via
  ...
  `dbus` + `openssl` + `pkg-config`. The flake's `buildInputs` provide all
  three; CI inherits them by running `nix build .#checks...`. _Never_ add a
  system-deps step (apt, brew, ...) to CI - if a crate needs a new system lib,
  add it to `buildInputs` in `flake.nix`.
